...
func runContainer(imageArg string) {
	...
	// Use unshare command like Python version
	containerInitCmd := fmt.Sprintf(`
		...
		export TERM=xterm;
		if [ -f /bin/sh ]; then exec /bin/sh -i; elif [ -f /bin/bash ]; then exec /bin/bash -i; else exec /usr/bin/sh -i; fi;
	`)
	
	cmd := exec.Command("unshare", "--uts", "--pid", "--net", "--mount", "--user", "--kill-child", "--map-root-user", fmt.Sprintf("--root=%s", containerRoot), "/bin/sh", "-c", containerInitCmd)
	cmd.Stdin = os.Stdin
